...
public struct Module {
    ...
    public init?(spmArguments: [String], spmName: String? = nil, inPath path: String = FileManager.default.currentDirectoryPath) {
        ...
        */
        ...
        if let firstBuildOutput = buildResults.string,
           let swiftArgs = SwiftPM.parseCompilerArguments(swiftBuildOutput: firstBuildOutput, moduleName: spmName),
            let actualModuleName = moduleName(fromArguments: swiftArgs) {
            fputs("Using module data from build output\n", stderr)
            self.init(name: actualModuleName, compilerArguments: swiftArgs)
            return
        }
        ...
        if let secondBuildOutput = secondBuildResults.string,
           let args = SwiftPM.parseCompilerArguments(swiftBuildOutput: secondBuildOutput, moduleName: spmName),
           let actualModuleName = moduleName(fromArguments: args) {
            self.init(name: actualModuleName, compilerArguments: args)
            return
        }
